...
class FER:
    ...
    def downsample_analyse(self, path_to_original_video: str, path_to_output: str, frame_width: int, frame_height: int, frame_skip_number: int):
        #240X426, 360x640.
        os.makedirs(path_to_output, exist_ok=True)
        ...
        self.analyze_video(path_to_video=f'{path_to_output}/downsample_{filename}', path_to_output=f'{path_to_output}/final_'+os.path.splitext(str(filename))[0], save_video=True, frame_skip=frame_skip_number)        
    ...
    def time_stamps_video(self, result_file):
        ...
        for i in range(len(data)):
            time_stamp_set.add(data[i]['time_stamp'])
        ...
        return dict_time_emot,time_stamp_set
    ...
    def speech_to_text(self, path_to_original_video):
        ...
        return sent, time_round, sent_id


    def match_time(self, path_to_original_video, fer_time, speech_time, sent_id, sent, dict_time_emot, path_to_output):
        filename = os.path.basename(path_to_original_video)
        final_ids = []
        final_sent = []
        
        match_set = set(fer_time).intersection(set(speech_time))
        match_list = sorted(match_set)
        match_emotions = [dict_time_emot[x] for x in match_list]
        dict_time_sent=dict(zip(sent_id,speech_time)) #time_round
        
        for i in match_list:
            if i in dict_time_sent.values():
                final_ids.append(list(dict_time_sent.values()).index(i))

        
        for i in final_ids:
            final_sent.append(sent[i])

        df = pd.DataFrame({'time_stamp': match_list, 'emotion': match_emotions, 'text':final_sent})
        df.to_csv(f'{path_to_output}/emotion_' + os.path.splitext(str(filename))[0] + '.csv', index = False)
        return  match_list, match_set, df
    
   

    #Version with downsampling

    def final_video_analyse(self, path_to_original_video: str, path_to_output: str, frame_skip_number: int, frame_width: int, frame_height: int):
        filename = os.path.basename(path_to_original_video)
        print("Downsampling of video started...")
        self.downsample_analyse(path_to_original_video, path_to_output, frame_width, frame_height, frame_skip_number)

  
        dict_time_emot, time_stamp_set = self.time_stamps_video(f'{path_to_output}/final_'+os.path.splitext(str(filename))[0] + '/result_ts.json')
        print("Timestamps and emotions extracted...")

        print("Speech to text analysis started...")
        
        sent, time_round, sent_id = self.speech_to_text(path_to_original_video)
        print("Converted video speech to text...")
        
        print("Final emotion file preparation started...")
        self.match_time(path_to_original_video, time_stamp_set, time_round, sent_id, sent, dict_time_emot, path_to_output)
